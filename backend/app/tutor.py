...
from .learning_context import CurrentLearningContext
...
async def handle_message(db: Session, learning_session: LearningSession, student: Student, text: str, context: CurrentLearningContext | None = None) -> TutorDecision:
    ...
    verified_context = json.dumps(context.to_dict(), ensure_ascii=False) if context else "{}"
    messages = [{"role": "system", "content": SYSTEM_PROMPT + f"\nStudent grade: {student.grade}; preferences: {json.dumps(student.preferences, ensure_ascii=False)}\nServer-verified learning context: {verified_context}\nTeaching strategy:\n{strategy}"}]
